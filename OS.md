...
+ 탐지(Detection) 및 회복(Recovery)
    - 시스템의 자원 할당 상태를 분석하여 데드락이 발생했는지 탐지한다.
    - 회복 방법
      - 우선순위가 낮은 프로세스를 종료하여 데드락 해소
      - 데드락 상태의 프로세스가 점유하고 있는 자원을 강제로 회수하여 다른 프로세스에 할당하여 해소
...
* 커널 수준 스레드 : __커널이 만들고 관리하는 스레드__
  * 커널이 직접 스케줄링하고 실행하기 때문에 커널의 관리 지원을 받을 수 있다.
  * 유저모드와 커널모드의 전환이 빈번하게 이뤄져 성능저하가 발생한다.
* 사용자 수준 스레드 : __스레드 관련 라이브러리로 구현해 사용하는 스레드__
  * __커널이 스레드를 모르기 때문에 모드 간의 전환이 없고 성능상 이득__
  * 커널이 각각을 개별 스레드로 인식하지 않기 때문에 하나의 스레드가 블록되면 모든 사용자 스레드가 블록된다.
...
프로세스가 CPU를 차지하고 사용중일 때, 우선순위가 높은 다른 프로세스가 들어온다면 CPU를 내주는 방식
...
CPU를 할당 받은 후, 프로세스가 종료될때까지 계속 CPU를 사용한다.
...
메모리가 부족할 때 프로세스 전체를 물리 메모리에서 디스크의 스왑 영역으로 내보내고, 필요할 때 다시 메모리로 가져오는 작업이다.
이 과정에서 시스템 성능 저하가 발생할 수 있다.
...
> 요구 페이징은 __프로그램 실행 시작 시에 프로그램 전체를 디스크에서 물리 메모리에 적재하는 대신, 초기에 필요한 것들만 적재하는 전략을 의미__ 한다. 따라서, 가상 메모리에는 실행과정에서 필요해질 때 페이지들이 적재되고 단 한번도 접근되지 않은 페이지는 물리 메모리로 적재되지 않는다.
...
이때는 메모리 자원을 늘리거나, 메모리 누수가 있는지 체크해봐야 한다.
...
* 파일 디스크립터는 프로세스의 파일 테이블과 운영체제의 파일 시스템을 연결하는 다리 역할
  * 이를 통해 프로세스는 파일을 읽거나 쓸 때, 이 정수 값(파일 디스크립터)을 사용하여 실제 파일에 접근
* 리눅스나 유닉스 시스템에서는 파일 디스크립터가 정수(Integer)로 표현되며, 프로세스가 파일을 열 때마다 새로운 파일 디스크립터가 할당

<br>

**프로세스가 파일을 열 때의 전체 동작 과정**
1. 프로세스가 open() 시스템 호출로 파일을 연다.
2. 운영체제가 파일을 확인하고 접근 권한을 검사
3. 운영체제는 파일 디스크립터를 생성하여 프로세스의 파일 테이블에 추가
  * 접근이 유효하면 파일 디스크립터를 생성
  * 파일 디스크립터는 프로세스의 파일 테이블에서 유일한 정수 값으로 관리되며 프로세스가 파일에 접근할 때 사용됨
  * 파일 디스크립터는 파일을 열 때마다 새로 생성되며 프로세스의 파일 디스크립터 테이블에 저장됨
4. 파일 디스크립터와 파일 테이블, inode가 연결되어 파일에 접근 가능
5. 프로세스는 파일 디스크립터를 사용하여 파일을 읽거나 쓴다.
6. 작업이 끝나면 close 호출로 파일을 닫고 리소스 해제
